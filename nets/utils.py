...
def image_transform(row, seed=100, contrast=(0.5, 2.5), bright=(-50, 50), dropout=0., shape=(64, 64), is_training=True): 
    (idx, row) = row[0], row[1]
    ...
    for bbox in np.loads(row.trible_box):
        ymin, xmin = bbox[0]
        ymax, xmax = bbox[1]
        #cv2.rectangle(new_bd_img, (ymin+padding, xmin+padding), (ymax+padding, xmax+padding), (0,0,255), 2)
        cascad_imgs.append(cv2.resize(new_bd_img[xmin+padding:xmax+padding, ymin+padding: ymax+padding], shape))
    #for idx, x in enumerate(cascad_imgs):
    #    cv2.imwrite("%s_%s_%s.jpg"%(row.age, row.gender, idx), x)
    #if idx > 10000:
    #    cv2.imwrite("%s_%s_%s_%s_%s.jpg"%(row.age, row.gender, row.roll, row.yaw, row.pitch), new_bd_img)
    return cascad_imgs
